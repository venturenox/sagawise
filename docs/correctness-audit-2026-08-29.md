...
- [ ] **5. Failure webhook has no timeout and blocks the reaper** — `reportFailure`, `instance_engine.go:323`.
...
- [ ] **6. DSL timeouts are not validated** — `ParseDSL`, `templating.go:55`.
...
- [ ] **7. `jsonMatches`/`jsonFirstMatch`/`markTask` swallow all errors** — `instance_engine.go:52–77`. **Root cause of much of this theme.**
...
- [ ] **8. Startup is log-and-continue; server serves half-initialized** — `ParseDSL`, `templating.go:114` (and the whole `else` block at `:39`).
...
- [ ] **10. List endpoint: capped at 10, errors become 404, values unescaped** — `ListWorkflowInstances`, `instance_engine.go:493`.
...
- [ ] **11. Rueidis client: dial error discarded, config split-brain** — `ConnectRueidis`, `db_connect.go:62`.
...
- [ ] **14. `/shutdown`: unauthenticated, destructive, and deadlocks** — `main.go:81`.
...
- [ ] **15. Node SDK: every non-retry call is a silent no-op** — `sdk/nodejs/sagawise.js:52` (also `:84`, `:115`).
...
- **`is_retry` parsed with `strconv.ParseBool` ignoring the error** — garbage silently becomes `false`.
